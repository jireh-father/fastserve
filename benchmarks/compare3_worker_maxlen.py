...
def run_vllm(model: str, n: int, speed_prompts: int, spec: str | None, max_len: int = 4096) -> dict:
    from vllm import LLM, SamplingParams

    max_new = 2048 if is_long_thinker(model) else 640
    mem_util = float(os.environ.get("FASTSERVE_GPU_MEM_UTIL", "0.6"))
    tp = int(os.environ.get("FASTSERVE_TP", "1"))
    kwargs = dict(model=model, max_model_len=max_len, gpu_memory_utilization=mem_util,
...
def main() -> None:
    ...
    res = (run_hf(args.model, args.n, args.speed_prompts) if args.mode == "hf"
           else run_vllm(args.model, args.n, args.speed_prompts, args.spec, args.max_len))
