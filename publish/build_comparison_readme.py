...
def build() -> str:
    ...
    lines += [
        "",
        "Speed in tok/s. The *vLLM* column is plain vLLM on the **original bf16 weights** (no "
        "quantization, no speculation) — the out-of-the-box baseline, not vLLM's ceiling; "
        "see the breakdown below for how much of the gap is quantization vs speculation. "
        "**fastserve is 3.8-8.7x faster than out-of-the-box serving at ~2-3x less memory** "
        "— accuracy held inside a 10pp gate (small "
...
def _ablation_section() -> str:
    """Split fastserve's gain over plain vLLM into quantization vs speculation."""
    return """### Where the speedup actually comes from
